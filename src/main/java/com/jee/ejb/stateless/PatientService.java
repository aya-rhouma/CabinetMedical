...
public class PatientService implements PatientServiceLocal {
    ...
    @Override
    public RendezVous reserverRendezVous(int patientId, int medecinId, LocalDate date, LocalTime heureDebut, LocalTime heureFin) {
        ...
        notificationService.notifyPatient(
                patientId,
                "Rappel RDV: votre rendez-vous est planifié le " + date + " de " + heureDebut + " à " + heureFin + "."
        );
        return rdv;
    }
    ...
    public RendezVous annulerRendezVous(int patientId, int rendezVousId) {
        RendezVous rdv = em.createQuery(
                        "SELECT r FROM RendezVous r WHERE r.id = :id AND r.patient.id = :patientId",
                        RendezVous.class
                ).setParameter("id", rendezVousId)
                .setParameter("patientId", patientId)
                .getSingleResult();
        rdv.setStatut("ANNULE");
        notificationService.notifyPatient(patientId, "RDV annulé: votre rendez-vous #" + rendezVousId + " a été annulé.");
        return rdv;
    }

    @Override
    public RendezVous modifierHoraireRendezVous(int patientId, int rendezVousId, LocalDate date, LocalTime heureDebut, LocalTime heureFin) {
        assertHoraireValide(date, heureDebut, heureFin);
        RendezVous rdv = em.createQuery(
                        "SELECT r FROM RendezVous r WHERE r.id = :id AND r.patient.id = :patientId",
                        RendezVous.class
        ...
        notificationService.notifyPatient(
                patientId,
                "Modification horaire: votre RDV #" + rendezVousId + " est déplacé au " + date + " (" + heureDebut + " - " + heureFin + ")."
        );
        return rdv;
    }
    ...
    public List<Medecin> getMedecinsBySpecialite(String specialite) {
        ...
        return em.createQuery(
                        "SELECT m FROM Medecin m WHERE LOWER(m.specialite) LIKE :specialite ORDER BY m.nom, m.prenom",
                        Medecin.class
                )
                .setParameter("specialite", "%" + s.toLowerCase() + "%")
                .getResultList();
    }

    @Override
    public List<Medecin> getMedecinsDisponibles(String specialite, LocalDate date, LocalTime heureDebut, LocalTime heureFin) {
        assertHoraireValide(date, heureDebut, heureFin);
        String s = specialite == null ? "" : specialite.trim();

        return em.createQuery(
                        "SELECT m FROM Medecin m " +
                                "WHERE (:specialite = '' OR LOWER(m.specialite) LIKE :likeSpecialite) " +
                                "AND m.id NOT IN (" +
                ...
                .setParameter("specialite", s)
                .setParameter("likeSpecialite", "%" + s.toLowerCase() + "%")
                .setParameter("date", date)
                .setParameter("heureDebut", heureDebut)
                .setParameter("heureFin", heureFin)
                .getResultList();
    ...
    public List<CertificatMedical> getDemandesCertificats(int patientId) {
        return em.createQuery(
                        "SELECT c FROM CertificatMedical c WHERE c.patient.id = :patientId ORDER BY c.id DESC",
    ...
    private void assertHoraireValide(LocalDate date, LocalTime heureDebut, LocalTime heureFin) {
        ...
        if (!heureDebut.isBefore(heureFin)) {
            throw new IllegalArgumentException("Heure début doit être avant heure fin.");
        }
    }
}
